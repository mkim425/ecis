...
#' @param ensembling A character string specifying a ensemble method, either "simple" or "linear_pool"; `c("simple", "linear_pool")`.
#' * When `"simple"` is specified, the ensemble is generated using the function selected in `agg_fun`, and
...
#' @param method A character string specifying algorithm for model importance calculation; `c("lomo", "lasomo")`.
...
ensemble_component_importance_score <- function(
                forecast_data,
                true_value,
                ensembling = c("simple", "linear_pool"),
                agg_fun = "mean",
                weighted = FALSE,
