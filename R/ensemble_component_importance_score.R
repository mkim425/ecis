#' Evaluate ensemble component model's importance based on a measure of their contribution to ensemble prediction accuracy for each combination of model task.
#'
...
#' @param ensemble_fun A character string specifying a ensemble method, either "simple_ensemble" or "linear_pool"; `c("simple_ensemble", "linear_pool")`.
#' * When `"simple_ensemble"` is specified, the ensemble is generated using the function selected in `agg_fun`, and
#' it takes into account the weight option specified in `weighted`.
#' * When `"linear_pool"` is specified, ensemble model outputs are created as a linear pool of component model outputs. This method supports only an `output_type` of `mean`, `quantile`, `cdf`, or `pmf`.
#' @param agg_fun A character string specifying aggregation method of component model outputs.
...
#' @param scoring_rule A character string specifying metric to use to calculate importance; `c("MAE", "MSE", "WIS", "CRPS", "Logscore")`.
#' Specify one of them depending on which is available for the output type in the input data.
#' @param na.action A character string specifying treatment for missing data; `c("worst", "average", "drop")`.
#'
...
ensemble_component_importance_score <- function(
        ...
        if (weighted != FALSE){
                ...
                if (num_forecast <= training_window_length){
                        stop("training_window_length should be smaller. Or,
                             more historical data is needed to estimate model weights
                             with the selected value of training_window_length.")
